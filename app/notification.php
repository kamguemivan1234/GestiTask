...
if (isset($_SESSION['role']) && isset($_SESSION['id'])) {
    ...
    // Helper function for time formatting - DÉFINIE AU DÉBUT
...
<style>
    ...
    /* Notification bell icon styling for header */
    .notification-bell {
        position: relative;
        cursor: pointer;
        transition: all 0.3s ease;
    ...
    <div class="notification-header">
        <h3 class="notification-title">
            ...
            <?php if ($unread_count > 0) { ?>
                <span class="notification-count"><?= $unread_count ?> new</span>
            <?php } ?>
        </h3>
    </div>
...
function markNotificationAsRead(notificationId, linkElement) {
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            // Mettre à jour le compteur global dans la barre de navigation
            ...
            // **NOUVELLE FONCTIONNALITÉ** : Fermer automatiquement le dropdown après lecture
            setTimeout(() => {
                ...
                // Fermer aussi le dropdown du header original
...
function updateNotificationBadge(count) {
    // **INTÉGRATION AVEC LE SYSTÈME HEADER EXISTANT**
    // Chercher le badge dans le header principal
    const headerBadge = document.querySelector("#notificationNum");
    
    if (count == 0) {
        // Aucune notification non lue - masquer le badge
        if (headerBadge) {
    ...
    // **COMPATIBILITÉ AVEC JQUERY SI DISPONIBLE**
    if (typeof $ !== 'undefined') {
    ...
    if (count == 0) {
        ...
        if (headerCount) {
            ...
            if (title) {
                ...
                newCount.textContent = count + ' new';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (dropdown) {
        ...
        // Ensure it's not hidden by parent overflow
    ...
    // Ajouter les événements de clic pour les notifications
    ...
    document.addEventListener('click', function(event) {
        ...
        // Si on clique en dehors du dropdown et que ce n'est pas le bouton de notification
